...
async function readJson(path, label) {
  try {
    ...
    if (err && typeof err === "object" && "code" in err && err.code === "ENOENT") {
      throw new Error(`${label} is missing. Run \`npm run check:sources\` (or \`npm run check:telegram\`) before \`npm run build\`.`);
    }
    throw err;
  }
}
...
async function main() {
  const sourcesDoc = await readJson(SOURCES_PATH, "data/sources.json");
  const latestDoc = await readJson(LATEST_PATH, "data/latest.json");
...
main().catch((err) => {
  console.error(`[generate-api] ${err instanceof Error ? err.message : String(err)}`);
  process.exit(1);
});
